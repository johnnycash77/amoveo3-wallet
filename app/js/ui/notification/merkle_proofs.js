function merkle_proofs_main() {
    function verify_callback(topHeader, tree, key, callback) {
        var topHeaderHash = hash(serialize_header(topHeader));
        variable_public_get(["proof", btoa(tree), key, btoa(array_to_string(topHeaderHash))], function(proof) {
            var val = 0;
            try {
                var val = verify_merkle(topHeader, key, proof);
            } catch(err) {
                console.error(err);
            }
            return callback(val);
    ...
    function verify_merkle(topHeader, trie_key, x) {
        //x is {return tree_roots, tree_root, value, proof_chain}
        var tree_roots = string_to_array(atob(x[1]));
        var header_trees_hash = string_to_array(atob(topHeader[3]));
    ...
    function serialize_tree_element(v, trie_key) {
        ...
        if ( t == "gov" ) {
            ...
            var amount = integer_to_array(128, 1).concat(integer_to_array(v[6], 5));
